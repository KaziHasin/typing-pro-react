...
export default function TypingTextbox() {
       ...
       const [wrontext, setWrongtext] = useState();
   ...
   useEffect(() => {
     ...
     const handleKeyDown = (event) => {
      ...
      const lastChar = inputValue[inputValue.length - 1];
       if (event.key === "Backspace") {
          ...
          console.log("text char "+text.charAt(textcount-1))



          if(lastChar !== text.charAt(textcount)) {
      ...
      // setBeforetextlength(typeTextCount)
    ...
    return (
        ...
        <div className = 'text-box bg-white shadow-lg flex justify-center align-items-center my-20' > 
          ...
          <div className="text text-6xl opacity-80 tracking-widest leading-loose p-8 relative">
               ...
               {list.map((list, index) => {
                  ...
                  if(index === textcount) {
                      return (
                        <span  key={index} className="blinking">{list}</span>
                      )
                  }

                 if(textcolor === "right" && index === (textcount - 1)) {
                    return (
                    <span  key={index} className="text-green-700 bg-green-50 mx-0.5 rounded">{list}</span>
                    )
                  }
                  ...
                  if(textcolor === "worng" && index === (textcount - 1)) {
                    return (
                      <>
                      <motion.span  key={index} className="text-red-700 bg-red-50 mx-0.5 rounded"
                      //  initial={{x: 10}}
                       animate={{ rotate: 180 }}
  transition={{ type: "spring", damping: 300 }}
                      >{list}</motion.span>
                        
                        <motion.span className='absolute text-red-700'
                          initial={{y: 10, opacity: 1}}
                          animate={{y: 200, opacity: 0}}
                          transition={{ duration: .7 }}

                        >{wrontext}</motion.span>
                  ...
                  if(beforetextright.indexOf(list) > -1 && beforetextrightlength.indexOf(index) > -1) {
                    return (
                    <span  key={index} className="text-green-700 bg-green-50 mx-0.5 rounded">{list}</span>
                    )
                  }
                  ...
                  if(beforetextwrong.indexOf(list) > -1 && beforetextwronglength.indexOf(index) > -1) {
                    return (
                    <span  key={index} className="text-red-700 bg-red-50 mx-0.5 rounded">{list}</span>
                    )
                  }
            ...
            <textarea id="textarea1" className="textarea1 border border-gray-400 pl-1" ref={inputRef}  onChange={(event) => setInputValue(event.target.value)}  onKeyDown={handleKeyDown}>
